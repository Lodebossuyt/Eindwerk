...
class Cart extends Model
{
    ...
    public function add($product, $product_id){
        ...
        $shopItems['product_image'] = $product->photos;
        $shopItems['product_body'] = $product->body;
        $this->totalQuantity++;
        $this->totalPrice += $product->price;
        $this->products[$product_id] = $shopItems; //op de array met key $product_id, wordt $shopItems gestoken
    }
    public function updateQuantity($id, $quantity){
        //telt het totaal aantal items in de winkelwagen
        $this->totalQuantity -= $this->products[$id]['quantity']; //bvb -5
        $this->totalQuantity += $quantity; //bvb +6
